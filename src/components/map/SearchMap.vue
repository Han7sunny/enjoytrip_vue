<template>
  ...
  <div class="gdlr-core-pbf-element">
          ...
          <div class="tourmaster-tour-search-field tourmaster-tour-search-field-tax">
            ...
            <div class="tourmaster-combobox-wrap">
              <select name="state" @change="selectState" v-model="state">
                <option :value="state.stateId" :key="state.stateId" v-for="state in stateList">
                  {{ state.stateName }}
                </option>
              </select>
            </div>
          ...
          <div class="tourmaster-tour-search-field tourmaster-tour-search-field-tax">
            ...
            <div class="tourmaster-combobox-wrap">
              <select name="city" v-model="city">
                <option :value="city.cityId" :key="city.cityId" v-for="city in cityList">
                  {{ city.cityName }}
                </option>
              </select>
            </div>
    ...
    <!-- <div class="container"> -->
      ...
      <div class="container alert alert-warning text-center" role="alert" id="null-alert">
...
export default {
  ...
  methods: {
    ...
    search() {
      this.contentSearchCriteria = {stateId : this.state, cityId : this.city, contentTypeId : this.contentType};console.log(this.contentSearchCriteria);
      getContentListByCriteria(
        this.contentSearchCriteria,
        // { stateId: this.state, cityId: this.city, contentTypeId: this.contentType },
        ({ data }) => {
          this.contentList = data;console.log(this.contentList);
        },
        (error) => {
          console.log(error); // errorpage
        }
    ...
    initMap() {
     ...
     this.kakaoMap = new kakao.maps.Map(mapContainer, mapOption); // 지도를 생성합니다
        ...
        // // 마커가 표시될 위치입니다
        // let markerPosition = new kakao.maps.LatLng(this.location.lat, this.location.lng);
...
// 검색결과 목록의 자식 Element를 제거하는 함수입니다
    ...
    getListItem(places) {
        ...
        var itemStr = '<span class="markerbg"><img src="./assets/images/marker_' + places.contentTypeId +
     ...
     removeMarker() {
      ...
      // 인포윈도우에 장소명을 표시합니다
       displayInfowindow(marker, title) { // 작성하기
